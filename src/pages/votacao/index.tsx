...
function VotacaoPage() {
  ...
  const enviarVoto = async () => {
    ...
    try {
      ...
      if (
        ...
        (message.includes("já votou") || message.includes("duplicado"))
      ) {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Se não tiver destino ou se soltou no mesmo lugar, não faz nada
    if (!destination || destination.index === source.index) return;

    // Salvamos os índices em variáveis para o TypeScript não se perder dentro do setOpcoes
    const sourceIndex = source.index;
    const destinationIndex = destination.index;

    setOpcoes((currentOpcoes) =>
      reorder(currentOpcoes, sourceIndex, destinationIndex)
    );
  };
    ...
    <Container fluid="xl" className="votacao-container">
      ...
      <div className="header-section text-center mb-5">
        <h2 className="votacao-title">
          🗳️ Campanhas de Votação{" "}
          {isSuperAdmin && (
            <small className="text-muted fs-6">(Modo Admin Geral)</small>
          )}
        </h2>
      ...
      {isLoading ? (
        ...
        <div className="votacao-grid">
          {campanhas.map((camp) => {
            // Correção TS: casting para any pois a interface pode não ter usuarioJaVotou
            const jaVotou = (camp as any).usuarioJaVotou;

            return (
              <Card
                key={camp.id}
                className={`votacao-card ${
                  !camp.ativo || jaVotou ? "disabled-card" : ""
                } ${jaVotou ? "voted-card" : ""}`}
                onClick={() => {
                  if (jaVotou) return;
              ...
              {opcoes.map((op) => (
                <div
                  ...
                  className={`opcao-card ${
                    selectedOpcoes.includes(op.id) ? "selected" : ""
                  }`}
      ...
      <Modal
        ...
        onHide={fecharModal}
        centered
        // Removido size="md" que causava erro no TS (padrão já é medium)
