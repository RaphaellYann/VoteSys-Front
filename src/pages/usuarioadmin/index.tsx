...
function UsuariosAdmin() {
  ...
  // Correção TS: Usando ChangeEvent<any> para aceitar tanto Input quanto Select
  const handleChangeForm = (e: ChangeEvent<any>) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
  };

  const handleSalvarUsuario = async (e: FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    setError(null);
