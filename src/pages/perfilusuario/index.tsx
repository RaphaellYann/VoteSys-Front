...
// Interfaces Locais
interface DadosFormState {
  nome: string;
  email: string;
  cpf: string;
}
interface SenhaFormState {
  senhaAtual: string;
  novaSenha: string;
...
function PerfilUsuario() {
  ...
  // @ts-ignore
  const { usuario, dataLogin, historicoNavegacao, ultimasVotacoes } =
    useSelector((state: RootState) => state.auth);

  // States
  const [showModalSenha, setShowModalSenha] = useState(false);
  const [dadosForm, setDadosForm] = useState<DadosFormState>({
    nome: "",
    email: "",
    cpf: "",
  });
  ...
  const [senhaForm, setSenhaForm] = useState<SenhaFormState>({
    senhaAtual: "",
    novaSenha: "",
    confirmarNovaSenha: "",
  });
  ...
  // --- HANDLERS ---
  const handleDadosChange = (e: ChangeEvent<HTMLInputElement>) => {
  ...
  const handleDadosSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!usuario || !usuario.id) return; // Verificação extra de segurança
    ...
    try {
      ...
      // CORREÇÃO AQUI: Adicionado "!" em usuario.id!
      const usuarioAtualizado = await atualizarProprioUsuario(
        usuario.id!,
        dadosParaEnviar
      );
  ...
  const handleSenhaSubmit = async (e: FormEvent) => {
    ...
    if (!usuario || !usuario.email) {
      setSenhaError(
        "Erro: E-mail do usuário não identificado. Faça login novamente."
      );
      return;
    }
    ...
    <Container fluid="xl" className="mt-5">
      <div className="mb-4">
        ...
        <p className="text-muted">
          Gerencie seus dados pessoais e visualize sua atividade.
        </p>
      </div>
                  ...
                  <Col md={6} className="border-end">
                    ...
                    <div className="mt-4">
                      <small className="text-muted">
                        Sessão iniciada em:{" "}
                        {dataLogin
                          ? new Date(dataLogin).toLocaleTimeString()
                          : "--:--"}
                      </small>
                    </div>
                  </Col>
                    ...
                    <div className="vote-list">
                      {ultimasVotacoes && ultimasVotacoes.length > 0 ? (
                        ultimasVotacoes.map((voto: any, i: number) => (
                          <div
                            key={i}
                            className="vote-item mb-3 pb-3 border-bottom"
                          >
                        ...
                        <Alert
                          variant="light"
                          className="text-center border text-muted small"
                        >
      ...
      >
        ...
        <Form onSubmit={handleSenhaSubmit}>
          ...
          <Modal.Footer>
            <Button variant="secondary" onClick={handleFecharModalSenha}>
              Cancelar
            </Button>
